...
class VideoDataset(Dataset):
    def __init__(self, dataset_path, images_path, clip_len):
        ...
        self.clip_len = clip_len # 生成的数据的深度的值
        ...
        # 获取对应视频的标签，并将标签转化为int阿数字类型，同时转化为array格式
    ...
    def __getitem__(self, index):
        ...
        buffer = self.crop(buffer, self.clip_len, self.crop_size) # 在数据的深度，高度，宽度方向进行随机裁剪，将数据转化为（clip_len, 112, 112, 3)
    ...
    def crop(self, buffer, clip_len, crop_size):
        time_index = np.random.randint(buffer.shape[0] - clip_len) # 生成一个深度方向上的随即长度
        height_index = np.random.randint(buffer.shape[1] - crop_size) # 生成一个高度方向上的随即长度
        width_index = np.random.randint(buffer.shape[2] - crop_size) # 生成一个宽度方向上的随即长度
