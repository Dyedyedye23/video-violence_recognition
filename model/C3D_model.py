...
class C3D(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        ...
        if pretrained:
            self.__load__pretrained_weight()
    ...
    def forward(self, x):
        ...
        x = self.relu(self.conv5b(x))  # 数据经过conv5b层后经过激活函数激活
        x = self.pool5(x)  # 数据经过pool5层进行池化操作

        x = x.reshape(-1, 8192) # 经过pool5池化以后特征图的大小为(512, 4, 4, 4)，利用view函数将其转化为(1, 8192)
    ...
    def __load__pretrained_weight(self):
        corresp_name = {
            # Conv1
